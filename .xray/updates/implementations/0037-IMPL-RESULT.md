...
| C02 | `REVISED` | Removed the Provider and handshake state; platform status now distinguishes loading, selected Cardano, responding accountless XRAY App, unavailable embedded host, and standalone modes. Current account places the payment address with account identity and exposes blockchain, network, and live block/epoch/slot tip in the detail grid, with a balance/native-asset summary; staking address is deliberately not exposed. The empty state preserves the same detail structure, while the support headline is blockchain-neutral. | UI state, data-exposure, and copy audit plus lint, typecheck, and build passed. |
...
| C04 | `REVISED` | The launcher/logger covers all demonstrated platform methods, including `getStatus`, plus direct Cardano v1 native and CIP-30 calls. It subscribes to all three v1 event scopes, so platform theme, currency, hide-balances, status, and route changes are logged alongside Cardano/CIP-30 events. Event entries use scope-qualified names and retain their complete event, payload, and context envelope; correlated request responses retain payload, context, and request ID. | Complete method/event inventory, event-envelope audit, typecheck, and production build passed. |
| C05 | `REVISED` | README and SKILLS teach only the no-handshake versioned API; stale Provider, capability, grouped-role, wire-family, and protocol-subpath APIs were removed. Home support guidance is blockchain-neutral and points generally to available bridge requests. | ESLint, stale scan, diff check, and production verification passed. |
